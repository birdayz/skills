...
Defaults: `--model gpt-5.6-sol`, `--effort ultra`, `--timeout 25m`. Override with `--model` / `--effort` / `--timeout`, or the `CODEX_REVIEW_MODEL` / `CODEX_REVIEW_REASONING_EFFORT` env vars. Other flags:
...
- **Asynchronous.** Launch the tool **backgrounded** (your harness's background-Bash mode) when you want to do other work while codex runs, or when the user wants to watch live. Here `-v` is appropriate: the live trace and heartbeats stream to the background process's output, which you poll on your own cadence. Read the final `VERDICT:` once it exits. Use **exactly one** level of backgrounding (harness background **or** `nohup … &`, never both) — the tool's own `--timeout` bounds the run.
  ...
  **Launch it as the harness background task DIRECTLY — the bare tool command, nothing around it.** `eval "$RUN" review --base <base> --bypass-sandbox` *is* the whole command you background. Do **not** add `timeout …` (the tool self-bounds and process-group-kills a stuck run), do **not** add `> file 2>&1` (the harness already captures the task's stdout, and the tool already writes `review.md`/`review.log`), do **not** add `| grep`/`| tail`. Your harness captures the tool's stdout — heartbeats **and** the final `VERDICT:` — and notifies you when the task exits; read *that captured output*. The instant you wrap the run to "track" it, you lose the heartbeat and then reach for the `pgrep`/`pkill` footguns below to recover — don't start down that path.

  **Two footguns when a review runs in the background — NEVER do either:**
  1. **Do NOT detect "is codex still running?" with `pgrep -f codex…`.** That pattern also matches your *own* monitor/waiter/`pkill` commands (their command lines contain the string `codexreview`/`codex`) — a **self-match** that reports "running" forever (or "done" spuriously when only your waiter is what matched). To know when a backgrounded review finished, use the **harness task's own completion signal** (you're notified on exit; the verdict is in its captured output) — not a process-name probe. If you truly must probe, match the *exact* binary+subcommand (`pgrep -f "codexreview.* review --base"`) and exclude your own PID — but prefer the completion signal.
  2. **Do NOT `pkill -f codex…` (or `pkill -f codexreview…`) while a review is up.** codex runs in your **session's process group**, so a stray cleanup `pkill` — even one aimed at a leftover waiter loop — terminates the live review mid-flight (`codexreview: interrupted; codex killed`), and you start over. Never `pkill` by the process name while a review is running. Let the tool's `--timeout` end a stuck run, or `TaskStop` the specific harness task by id.
...
`18.4k tokens · 47 tool-calls` · Generated by Codex 0.139.0 on gpt-5.6-sol (ultra) and [codex-review](https://github.com/birdayz/skills/tree/master/codex-review)
```
...
- **Never `pgrep`/`pkill` on the codex process name to track or clean up a backgrounded run.** `pgrep -f codex…` self-matches your own monitor/waiter/`pkill` commands (their command lines contain the string) → it reports "running" forever or "done" spuriously; and `pkill -f codex…` kills the review itself — codex shares your session's process group, so a cleanup `pkill` (even aimed at a leftover waiter) terminates the live review (`interrupted; codex killed`). Use the harness task's completion signal to know it's done, and `TaskStop <task-id>` (never `pkill`) to cancel a specific run. (See **Sync vs async → Two footguns**.)
