...
# Time the run — the duration goes in the posted comment header. All /tmp/codex-*
# files are per-run: this block overwrites them fresh on every invocation.
start=$(date +%s)
...
grep -q '[^[:space:]]' /tmp/codex-review.md 2>/dev/null || { echo "review is empty — see /tmp/codex-review.log; not posting"; exit 1; }

pr=$(gh pr view --json number -q .number)            # current branch's PR
sha=$(git rev-parse --short HEAD)
duration=$(cat /tmp/codex-duration.txt 2>/dev/null)                      # written in Step 1
...
gh pr comment "$pr" --body "$(printf '**Codex finished review%s** · `%s`\n\n---\n\n%s\n\n---\n\nGenerated by Codex %s and [codex-review](%s)' \
  "$dur_phrase" "$sha" "$(cat /tmp/codex-review.md)" "$version" "$skill_url")"
```
...
Generated by Codex 0.55.0 and [codex-review](https://github.com/birdayz/skills/tree/master/codex-review)
```
...
grep -q '[^[:space:]]' /tmp/codex-review.md 2>/dev/null || { echo "review is empty — see /tmp/codex-review.log; not posting"; exit 1; }
dur=$(( $(date +%s) - start )); duration="$((dur/60))m$((dur%60))s"; dur_phrase=${duration:+ in $duration}   # self-contained: no /tmp handoff needed
sha=$(git rev-parse --short HEAD)
